...
when 'package'
  ...
  #Download deb file from the internet

  
  remote_file 'Download remote file' do
    path /tmp/
    source http://ftp.de.debian.org/debian/pool/main/m/mariadb-10.1/mariadb-server-10.1_10.1.35-1_amd64.deb
    mode '0755'
    action :create
  end
  ...
  case node['platform']
  when 'debian','ubuntu'
    execute 'debian_install-mariadb' do
      command 'sudo apt-get install mariadb-server -y'
    end
  when 'redhat', 'centos', 'scientific', 'amazon'                                            
